...
layout: default
nav_order: 60
description: The ECG module provides waveform data, and lookup tables which can be
  used to link subjects to other MIMIC-IV modules via `subject_id`. A `study_id` allows
  for linking within the ECG module while `note_id` allows linking to the cardiologist
  note in the MIMIC-IV-Note module.
has_children: true
parent: MIMIC-IV
---
